  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(135,89,255,0.12),transparent_28%),radial-gradient(circle_at_82%_18%,rgba(94,234,212,0.06),transparent_22%),linear-gradient(180deg,#05060a_0%,#080a11_42%,#05060a_100%)] px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        {step === 'orientation' && (
          <div className="rounded-[1.8rem] border border-white/8 bg-white/[0.03] p-6 xl:p-8">
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button
                onClick={() => setStep('context')}
                className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/[0.05] px-6 py-3 text-sm font-semibold text-white/80 transition hover:border-white/16 hover:bg-white/[0.08]"
              >
        ...
        {step === 'context' && (
          <div className="grid gap-4 xl:grid-cols-[1fr_0.48fr]">
              ...
              <div className="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button onClick={() => setGoal('clarity')} className={`px-4 py-2 rounded-full ${goal === 'clarity' ? 'bg-white text-black' : 'bg-white/[0.04] text-white/80'}`}>Clarity</button>
                    <button onClick={() => setGoal('rewrite')} className={`px-4 py-2 rounded-full ${goal === 'rewrite' ? 'bg-white text-black' : 'bg-white/[0.04] text-white/80'}`}>Rewrite</button>
                    <button onClick={() => setGoal('pattern')} className={`px-4 py-2 rounded-full ${goal === 'pattern' ? 'bg-white text-black' : 'bg-white/[0.04] text-white/80'}`}>Pattern</button>
                  </div>
                </div>
              </div>

              <div className="mt-6 flex gap-3">
                <button onClick={() => setStep('orientation')} className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/[0.05] px-6 py-3 text-sm font-semibold text-white/80">Back</button>
            ...
            <div className="rounded-[1.8rem] border border-white/8 bg-gradient-to-br from-primary/8 to-black/12 p-6">
        ...
        {step === 'complete' && (
          <div className="rounded-[1.8rem] border border-primary/16 bg-gradient-to-br from-primary/12 via-primary/6 to-black/16 p-6 text-center">
